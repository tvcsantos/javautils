    ...
    protected ByteBuffer doInBackground() throws Exception {
        ...
        if (postParameters != null) {
            ...
            connection.addRequestProperty("Content-Length", 
                    String.valueOf(postData.remaining()));
            ...
            WritableByteChannel out = 
                    Channels.newChannel(connection.getOutputStream());
            out.write(postData);
            out.close();
        }
        ...
        ByteBufferOutputStream buffer = new ByteBufferOutputStream(
                (int) (contentLength > 0 ? contentLength : 32 * 1024));
        ...
        try {
            while (!isCancelled() && ((buffer.transferFrom(in)) >= 0)) {
                ...
                if (isContentLengthKnown()) {
                    setProgress(
                            (int) ((buffer.position() * 100) / contentLength));
                }
            }
        } catch (IOException e) {
            // if the content length is not known in 
            // advance an IOException (Premature EOF) 
